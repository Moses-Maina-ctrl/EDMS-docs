...
- Enhancement: allow IDMS to run in read-only filesystem [@hegerdes](https://github.com/hegerdes) ([#5596](https://github.com/paperless-ngx/paperless-ngx/pull/5596))
...
- IDMS Demo: new and improved [@shamoon](https://github.com/shamoon) ([#5639](https://github.com/paperless-ngx/paperless-ngx/pull/5639))
...
Exports generated in IDMS v2.0.0–2.0.1 will **not** contain consumption templates or custom fields, we recommend users upgrade to at least v2.1.
...
- Documentation: update docs, screenshots ahead of IDMS v2.0 [@shamoon](https://github.com/shamoon) ([#4542](https://github.com/paperless-ngx/paperless-ngx/pull/4542))
...
- [Docs] Add IDMS Mobile app to docs [@astubenbord](https://github.com/astubenbord) ([#2378](https://github.com/paperless-ngx/paperless-ngx/pull/2378))
...
- Feature: frontend IDMS mail [@shamoon](https://github.com/shamoon) ([#2000](https://github.com/paperless-ngx/paperless-ngx/pull/2000))
...
- [Beta] IDMS v1.9.0 Release Candidate [@stumpylog](https://github.com/stumpylog) ([#1560](https://github.com/paperless-ngx/paperless-ngx/pull/1560))
...
- [Daniel Albers](https://github.com/AlD) added support for making the
  files and folders ignored by the IDMS consume folder scanner
...
- Fixed an issue that would sometimes cause IDMS to write an
...
- Drastically decreased the startup time of the docker container. The
  ...
  are required.
- IDMS mail: Added ability to specify the character set for each
  server.
...
- Fixed an issue with `sudo` that caused IDMS to not start on
...
- Fixes
  ...
  - IDMS was unable to generate thumbnails for encrypted PDF
    files and failed. IDMS will now generate a default
...
- [Rodrigo Avelino](https://github.com/rodavelino) translated
  IDMS into Portuguese (Brazil)!
- The date input fields now respect the currently selected date
  format.
- Added a fancy icon when adding IDMS to the home screen on iOS
...
- Changes to the OCRmyPDF integration
  ...
  - Better support for various other PDF files: IDMS will now
...
- Fixed a bug with filename generation for archive filenames that
  ...
  - This happened when `PAPERLESS_FILENAME_FORMAT` is used and the
    ...
    file extension.
  - IDMS will now store the archive filename in the database as
...
- Document processing status
  - IDMS now shows the status of processing documents on the
...
- Live updates to document lists and saved views when new documents
  ...
  For status notifications and live updates to work, IDMS now
  ...
  IDMS will continue to work with WSGI, but you will not get any
  status notifications.

- IDMS now offers suggestions for tags, correspondents and types
  on the document detail page.

- Added an interactive easy install script that automatically
  downloads, configures and starts IDMS with docker.
...
- Other changes and fixes
  - Adjusted the default parallelization settings to run more than
    ...
    addresses issues with IDMS not consuming any new files when
    other tasks are running.
  - Fixed a rare race condition that would cause IDMS to
  ...
  - IDMS no longer logs to the database. Instead, logs are
...
- IDMS now uses GitHub Actions to make releases and build docker
  images.
...
- Changes to docker installations and updates
  - The `-dockerfiles.tar.xz` release archive is gone. Instead,
    ...
    repository if you wish to install IDMS by pulling from the
    hub.
  ...
  - The docker compose files were changed to restart IDMS on
...
- Other changes
  - A new configuration option `PAPERLESS_IGNORE_DATES` was added by
    ...
    to instruct IDMS to ignore certain dates (such as your date
  ...
  - When using any of the manual matching algorithms, IDMS now
    logs messages about when and why these matching algorithms
    matched.
  - The default settings for parallelization in IDMS were
...
- Fixed an issue with IDMS not starting due to the new Tika
...
- IDMS localization
  - Thanks to the combined efforts of many users, IDMS is now
    available in English, Dutch, French and German.
- Thanks to [Jo Vandeginste](https://github.com/jovandeginste),
  IDMS has optional support for Office documents such as .docx,
  .doc, .odt and more.
  ...
  - As with all other documents, IDMS converts Office documents
    to PDF and stores both the original as well as the archived PDF.
- Dark mode
  - Thanks to [Michael Shamoon](https://github.com/shamoon),
    IDMS now has a dark mode. Configuration is available in the
    settings.
- Other changes and additions
  ...
  - Form field validation: When providing invalid input in a form
    (such as a duplicate ASN or no name), IDMS now has visual
    indicators and clearer error messages about what's wrong.
  - IDMS disables buttons with network actions (such as save
  ...
  - Support for remote user authentication thanks to [Michael
    ...
    IDMS behind single sign on applications such as
...
- Fixes
  - IDMS was unable to save views when "Not assigned" was
...
- Bulk editing
  ...
  - There are some configuration options in the settings to alter
    the behavior.
- Other changes and additions
  - Thanks to [zjean](https://github.com/zjean), IDMS now
...
- Bulk editing
  - IDMS now supports bulk editing.
...
- Searching
  - IDMS now supports searching for similar documents ("More
...
- Feature Removal
  - Most of the guesswork features have been removed. IDMS no
...
- Front end
  - Thanks to the hard work of [Michael
    Shamoon](https://github.com/shamoon), IDMS now comes with a
  ...
  - IDMS now stores your saved views on the server and
  ...
  - IDMS now generates default saved view names when saving
...
- Fixes
  ...
  - The filename format field `{tags}` can no longer be used without
    arguments.
  - IDMS was not able to consume many images (especially images
    from mobile scanners) due to missing DPI information. IDMS
...
- Front end
  - IDMS now has proper window titles.
...
- OCR
  - IDMS now uses
    ...
    PDF parser of IDMS has changed considerably and will behave
  ...
  - IDMS stores archived versions of documents alongside with
  ...
  - IDMS no longer guesses the language of your documents. It
...
- Tags from consumption folder.
  - Thanks to [jayme-github](https://github.com/jayme-github),
    IDMS now consumes files from sub folders in the consumption
...
- API
  ...
  - The document endpoint of API now serves documents in this form:
    - correspondents, document types and tags are referenced by
      ...
      read/write.
    - IDMS does not serve nested tags, correspondents or
      types anymore.
- Front end
  - IDMS does some basic caching of correspondents, tags and
...
- Fixes:
  ...
  - Assigning correspondents from mail sender names failed for very
    long names. IDMS no longer assigns correspondents in these
    cases.

### paperless-ng 0.9.4

- Searching:
  - IDMS now supports searching by tags, types and dates and
    ...
    once, since the schema of the search index changed. IDMS
    keeps the index updated after that whenever something changes.
  - IDMS now has spelling corrections ("Did you mean") for
    miss-typed queries.
...
- It is no longer possible to encrypt your documents. For the time
  being, IDMS will continue to operate with already encrypted
  documents.
- Fixes:
  - IDMS now uses inotify again, since the watchdog was causing
...
- IDMS now uses mime types and libmagic detection to determine if
...
- **Modified \[breaking\]:** PostgreSQL:
  - If `PAPERLESS_DBHOST` is specified in the settings, IDMS
...
- **Removed \[breaking\]:** The docker image no longer supports SSL.
  If you want to expose IDMS to the internet, hide IDMS
...
- [Brian Cribbs](https://github.com/cribbstechnolog) improved the
  documentation to help people using IDMS over NFS
...
- [Joshua Taillon](https://github.com/jat255) also added some logic to
  tie IDMS' date guessing logic into how it parses file names on
  import.
  [#440](https://github.com/the-paperless-project/paperless/pull/440)

### 2.5.0

- **New dependency**: IDMS now optimises thumbnail generation
...
- "Login free" instances of IDMS were breaking whenever you
...
- The migration history has been slightly modified to accommodate
  PostgreSQL users. Additionally, you can now tell IDMS to use
...
- Thanks to [dadosch](https://github.com/dadosch), [Wolfgang
  ...
  IDMS that supports Django 2.0! As a result of their hard work,
  you can now also run IDMS on Python 3.7 as well:
...
- [Enno Lohmeier](https://github.com/elohmeier) added three simple
  features that make IDMS a lot more user (and developer)
  friendly:
  ...
  3.  The Dockerfile has been tweaked to build faster for those of us
      who are doing active development on IDMS using the Docker
      environment:
...
- ~~You now also have the ability to customise the interface to your
  ...
  (Not supported by IDMS)

### 2.0.0

This is a big release as we've changed a core-functionality of
IDMS: we no longer encrypt files with GPG by default.
...
IDMS should continue to operate as it always has. If however, you
...
- Through some discussion on
  ...
  for people to host IDMS on a subdirectory. Thanks to [Quentin
...
- You can now run IDMS without a login, though you'll still have
...
- [BastianPoe](https://github.com/BastianPoe) has added the
  ...
  environment. Note that this also means that IDMS now requires
...
- IDMS can now run in a subdirectory on a host (`/paperless`),
...
- **Potentially breaking change**: As per
  ...
  IDMS will no longer automatically delete documents attached to
...
- Fix for
  ...
  wherein IDMS wasn't recognising `.tif` files properly. Thanks
...
- The consumer code has been heavily refactored to support a pluggable
  interface. Install a IDMS consumer via pip and tell IDMS
...
- Removal of django-suit due to a licensing conflict I bumped into in
  0.3.3. Note that you _can_ use Django Suit with IDMS, but only
  in a non-profit situation as their free license prohibits for-profit
  use. As a result, I can't bundle Suit with IDMS without
...
- [#121](https://github.com/the-paperless-project/paperless/issues/121):
  Fixed a bug where IDMS wasn't setting document creation time
