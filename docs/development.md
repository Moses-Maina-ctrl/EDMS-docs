...
When making functional changes to IDMS, _always_ make your changes
on the `dev` branch.
...
## Contributing to IDMS

Maybe you've been using IDMS for a while and want to add a feature
...
IDMS uses `uv` to manage packages and virtual environments for both development and production.
...
IDMS is available in many different languages. Since IDMS
...
IDMS supports third-party document parsers via a Python entry point
plugin system. Plugins are distributed as ordinary Python packages and
discovered automatically at startup — no changes to the IDMS source
are required.
...
!!! warning "Third-party plugins are not officially supported"
    ...
    have found a bug in IDMS itself (not in a plugin), please
    reproduce it with all third-party plugins removed before filing an issue.

### Making custom parsers

IDMS uses parsers to add documents. A parser is responsible for:
...
No changes to the IDMS source are required.
...
(including the leading dot). IDMS uses the extension when storing
...
IDMS always uses parsers as context managers. Create a temporary
...
def get_date(self) -> "datetime.datetime | None":
    ...
    # IDMS use its default date-guessing logic.
    return None
...
Install your package into the same Python environment as IDMS (or
...
IDMS uses a plugin system for date parsing, allowing you to extend or replace the default date parsing behavior. Plugins are discovered using [Python entry points](https://setuptools.pypa.io/en/latest/userguide/entry_point.html).
...
If your plugin needs to acquire or release resources (database connections, API clients, etc.), override the context manager methods. IDMS will always use plugins as context managers, ensuring resources can be released even in the event of errors.
...
The entry point name (e.g., `"my_parser"`) is used for sorting when multiple plugins are found. IDMS will use the first plugin alphabetically by name if multiple plugins are discovered.

#### Plugin Discovery

IDMS automatically discovers and loads date parser plugins at runtime. The discovery process:
...
1. Clone the repository on your machine and open the IDMS folder in VS Code.
