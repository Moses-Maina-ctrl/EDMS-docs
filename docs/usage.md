...
IDMS is an application that manages your personal documents. With
the (optional) help of a document scanner (see [the scanners wiki](https://github.com/paperless-ngx/paperless-ngx/wiki/Scanner-&-Software-Recommendations)), IDMS transforms your unwieldy
physical documents into a searchable archive and provides many utilities
for finding and managing your documents.

## Terms and definitions

IDMS essentially consists of two different parts for managing your
documents:
...
- A _tag_ is a label that you can assign to documents. Think of labels
  ...
  implemented in IDMS is simply that tags are much more versatile
  than folders.
...
- IDMS also supports _custom fields_ which can be used to
  store additional metadata about a document.

## The Web UI

The web UI is the primary way to interact with IDMS. It is a
...
a button to upload documents to IDMS but you can also drag and
...
IDMS includes management lists for tags, correspondents, document types
...
IDMS v2.19 introduces support for nested tags, allowing you to create a
...
## Adding documents to IDMS

Once you've got IDMS setup, you need to start feeding documents
into it. When adding documents to IDMS, it will perform the
...
2.  IDMS will create an archivable PDF/A document from your
    document. If this document is coming from your scanner, it will have
    embedded selectable text.
3.  IDMS performs automatic matching of tags, correspondents and
...
!!! tip
    ...
    IDMS to create archived versions for born-digital documents, set
...
!!! note
    ...
    No matter which options you choose, IDMS will always store the
    ...
    consumption directory will stored inside the IDMS file
...
inside IDMS and removed from the consumption folder.

Getting stuff into this directory is up to you. If you're running
IDMS on your local computer, you might just want to drag and drop
...
!!! warning
    ...
    IDMS file structure using any settings / storage paths
...
The dashboard has a button to upload documents to IDMS or you
...
software (e.g. for mobile devices) that is compatible with IDMS.
...
4.  If so, instruct IDMS to consume the attachments and optionally
    use the metadata provided in the rule for the new document.
5.  If documents were consumed from a mail, the rule action is performed
    on that mail.

IDMS will check all emails only once and completely ignore messages
...
- **Delete:** Immediately deletes mail that IDMS has consumed
  documents from. Use with caution.
- **Mark as read:** Mark consumed mail as read. IDMS will not
  consume documents from already read mails. If you read a mail before
  IDMS sees it, it will be ignored.
- **Flag:** Sets the 'important' flag on mails with consumed
  documents. IDMS will not consume flagged mails.
- **Move to folder:** Moves consumed mails out of the way so that
  IDMS won't consume them again.
- **Add custom Tag:** Adds a custom tag to mails with consumed
  documents (the IMAP standard calls these "keywords"). IDMS
...
!!! note
    ...
    documents from and IDMS will read them automatically. The default
    action "mark as read" is pretty tame and will not cause any damage or
    data loss whatsoever.

    You can also setup a special folder in your mail account for IDMS
    and use your favorite mail client to move to be consumed mails into that
    folder automatically or manually and tell IDMS to move them to yet
...
!!! note
    ...
    fetching mail from a certain folder, check the IDMS logs. When a
    folder is not located, IDMS will attempt to list all folders found
    in the account to the IDMS logs.

!!! note

    IDMS will process the rules in the order defined in the admin page.
...
IDMS is set up to check your mails every 10 minutes. This can be
configured via [`PAPERLESS_EMAIL_TASK_CRON`](configuration.md#PAPERLESS_EMAIL_TASK_CRON)

#### Processed Mail

IDMS keeps track of emails it has processed in order to avoid processing the same mail multiple times. This uses the message `UID` provided by the mail server, which should be unique for each message. You can view and manage processed mails from the web UI under Mail > Processed Mails. If you need to re-process a message, you can delete the corresponding processed mail entry, which will allow IDMS to process the email again the next time the mail fetch task runs.

#### OAuth Email Setup

IDMS supports OAuth2 authentication for Gmail and Outlook email accounts. To set up an email account with OAuth2, you will need to create a 'developer' app with the respective provider and obtain the client ID and client secret and set the appropriate [configuration variables](configuration.md#email_oauth). You will also need to set either [`PAPERLESS_OAUTH_CALLBACK_BASE_URL`](configuration.md#PAPERLESS_OAUTH_CALLBACK_BASE_URL) or [`PAPERLESS_URL`](configuration.md#PAPERLESS_URL) to the correct value for the OAuth2 flow to work correctly.
...
Once setup, navigating to the email settings page in IDMS will allow you to add an email account for Gmail or Outlook using OAuth2. After authenticating, you will be presented with the newly-created account where you will need to enter and save your email address. After this, the account will work as any other email account in IDMS and refreshing tokens will be handled automatically.
...
If enabled, IDMS can use an AI LLM model to suggest document titles, dates, tags,
...
IDMS can use an AI LLM model to answer questions about a document or across multiple documents. Again, this feature works best when RAG is enabled. The chat feature is available in the upper app toolbar and will switch between chatting across multiple documents or a single document based on the current view.

## Sharing documents from IDMS

IDMS supports sharing documents with other users by assigning them [permissions](#object-permissions)
...
- From the bulk editor you can create a **share link bundle** for any selection. IDMS prepares a ZIP archive in the background and exposes a single share link. You can revisit the "Manage share link bundles" dialog to monitor progress, retry failed bundles, or delete links.
...
IDMS supports directly sending documents via email. If an email server has been [configured](configuration.md#email-sending)
...
Permissions in IDMS are based around ['global' permissions](#global-permissions) as well as
...
IDMS supports editing users and groups via the 'frontend' UI, which can be found under
...
Admin status (Django 'staff status') grants access to viewing the IDMS logs and the system status dialog
...
Workflows allow hooking into the IDMS document pipeline, for example to alter what metadata (tags, doc types) and
...
flowchart TD
    ...
    consumption --> |No| D
    C --> D[Document Added]
    D -- IDMS 'matching' of tags, etc. --> added
...
IDMS supports the use of custom fields for documents as of v2.0, allowing a user
...
IDMS supports basic editing operations for PDFs (these operations currently cannot be performed on non-PDF files). When viewing an individual document you can
...
!!! important
    ...
    Note that rotation and deleting pages alter the IDMS _original_ file, which would, for example, invalidate a digital signature.

## Document History

As of version 2.7, IDMS automatically records all changes to a document and records this in an audit log. The feature requires [`PAPERLESS_AUDIT_LOG_ENABLED`](configuration.md#PAPERLESS_AUDIT_LOG_ENABLED) be enabled, which it is by default as of version 2.7.
...
IDMS offers a couple tools that help you organize your document
...
binder, you can recreate that system in IDMS by defining a document
...
IDMS by creating tags and assigning them to relevant documents.
...
!!! tip
    ...
    sorted into various folders. With the tag system of IDMS, you can
...
- A tag `bank account x` for all bank statement related to that
  account.
- A tag `mail` for anything that you added to IDMS via its mail
...
objects IDMS uses, including documents, tags, workflows, etc. Only
...
IDMS offers an extensive searching mechanism that is designed to
...
When you search IDMS for a document, it tries to match this query
against your documents. IDMS will look for matching documents by
inspecting their content, title, correspondent, type, tags, notes, and
custom field values. IDMS returns a scored list of results, so that
documents matching your query better will appear further up in the search
results.

By default, IDMS returns only documents which contain all words
...
IDMS also offers advanced search syntax if you want to drill down further.
...
learn more about the query language used by IDMS, see the
...
!!! note
    ...
    When enabled, IDMS will include near-miss results ranked below exact matches.
...
based on which area of IDMS you are using.

## The recommended workflow {#usage-recommended-workflow}

Once you have familiarized yourself with IDMS and are ready to use
...
!!! tip
    ...
    This also enables IDMS to automatically parse and process the ASN
...
1.  Searching in IDMS for the document.
...
### Processing of documents in IDMS

Once you have scanned in a document, proceed in IDMS as follows.
...
5.  Check that the date of the document is correct. IDMS tries to
...
!!! tip
    ...
    IDMS will assign them automatically. After consuming a couple
    documents, you can even ask IDMS to *learn* when to assign tags and
...
- If you scan and process your documents in IDMS regularly,
  ...
  this tag.
- If you do not scan documents regularly and use IDMS solely for
...
IDMS supports performing OCR on documents using remote services. At the moment, this is limited to
[Microsoft's Azure "Document Intelligence" service](https://azure.microsoft.com/en-us/products/ai-services/ai-document-intelligence).
This is of course a paid service (with a free tier) which requires an Azure account and subscription. Azure AI is not affiliated with
IDMS in any way. When enabled, IDMS will automatically send appropriate documents to Azure for OCR processing, bypassing
...
IDMS consists of the following components:
...
- **The task processor:** IDMS relies on [Celery - Distributed
  ...
  - Maintaining the search index and the automatic matching
    algorithm. These are things that IDMS needs to do from time
    to time in order to operate properly.

  This allows IDMS to process multiple documents from your
...
- Optional: A database server. IDMS supports PostgreSQL, MariaDB
