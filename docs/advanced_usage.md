...
IDMS offers a couple of features that automate certain tasks and make
your life easier.

## Matching tags, correspondents, document types, and storage paths {#matching}

IDMS will compare the matching algorithms defined by every tag,
...
a `matching_algorithm` of `Exact`, IDMS will automatically tag
...
IDMS comes with a new matching algorithm called _Auto_. This
...
IDMS tries to hide much of the involved complexity with this
...
- Changes to your documents are not immediately reflected by the
  ...
  documents after changes. IDMS periodically (default: once each
...
- IDMS also needs a reasonable amount of negative examples to
  decide when not to assign a certain tag, correspondent, document
  type, or storage path. This will usually be the case as you start
  filling up IDMS with documents. Example: If all your documents
  are either from "Webshop" or "Bank", IDMS will assign one
...
consumed. Rather than try to predict what you may want to do, IDMS
lets you execute scripts of your own choosing just before or after a
document is consumed using a couple of simple hooks.

Just write a script, put it somewhere that IDMS can read & execute,
...
By default, IDMS stores your documents in the media directory and
...
or using [storage paths (see below)](#storage-paths). IDMS adds the
...
!!! warning
    ...
    Do not manually move your files in the media folder. IDMS remembers
    the last filename a document was stored as. If you do rename a file,
    IDMS will report your files as missing and won't be able to find
    them.

!!! tip

    IDMS checks the filename of a document whenever it is saved. Changing (or deleting)
...
IDMS provides the following variables for use within filenames:
...
- `{{ doc_pk }}`: The IDMS identifier (primary key) for the document.
...
IDMS will try to conserve the information from your database as
...
If IDMS detects that two documents share the same filename,
IDMS will automatically append `_01`, `_02`, etc to the filename.
...
IDMS will fall back to using the default naming scheme instead.

!!! caution

    As of now, you could potentially tell IDMS to store your files anywhere
...
IDMS will attempt to "clean" certain invalid PDFs with `qpdf` before processing if, for example, the mime_type
...
Third-party parser plugins extend IDMS to support additional file
...
!!! warning "Third-party plugins are not officially supported"
    ...
    The IDMS maintainers do not provide support for third-party
...
available when IDMS discovers plugins at startup.

### Bare metal

Install the package into the same Python environment that runs IDMS.
...
Restart all IDMS services after installation so the new plugin is
discovered.
...
!!! warning
    ...
    an older system may fix issues that can arise while setting up IDMS but
...
IDMS is able to utilize barcodes for automatically performing some tasks. Barcodes are only supported for PDF documents or TIFF, [if enabled](configuration.md#PAPERLESS_CONSUMER_BARCODE_TIFF_SUPPORT).
...
For usage in IDMS, the type of barcode does not matter, only the contents of it.
...
If document splitting is enabled, IDMS splits _after_ a separator barcode by default.
This means:
...
When enabled, IDMS will parse barcodes and attempt to interpret and assign tags.
...
keep in mind that IDMS will _not_ automatically create the directory for you.)
IDMS will then process the scan and move it into an internal staging area.
...
scan is copied into the sub-directory, IDMS will collate the previous scan with the
...
IDMS will notice when the first, "odd numbered" scan has less pages than the second
scan (this can happen when e.g. the ADF skipped a few pages in the first pass). In that
case, IDMS will remove the staging copy as well as the scan, and give you an error
...
You therefore need to make sure that IDMS is running while you upload the files into
...
a day later to scan a new double-sided document, IDMS will only keep an "odd numbered
...
## SSO and third party authentication with IDMS

IDMS has a built-in authentication system from Django but you can easily integrate an
...
Version 2.5.0 of IDMS added support for integrating other authentication systems via
...
To associate an existing IDMS account with a social account, first login with your
...
{"openid_connect": {"APPS": [{"provider_id": "keycloak","name": "Keycloak","client_id": "IDMS","secret": "<CLIENT_SECRET>","settings": { "server_url": "https://<KEYCLOAK_SERVER>/realms/<REALM>/.well-known/openid-configuration"}}]}}'
```
...
IDMS can be configured to decrypt gpg encrypted emails before consumption.
...
drwx------ 1 IDMS IDMS   4096 Aug 18 17:52 .
drwxr-xr-x 1 IDMS IDMS   4096 Aug 18 17:52 ..
srw------- 1 IDMS IDMS      0 Aug 18 17:22 S.gpg-agent
-rw------- 1 IDMS IDMS 147940 Jul 24 10:23 pubring.gpg
```
