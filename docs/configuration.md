...
IDMS provides a wide range of customizations. Depending on how you
run IDMS, these settings have to be defined in different places.
...
- If you run IDMS on docker, `paperless.conf` is not used.
  Rather, configure IDMS by copying necessary options to
  `docker-compose.env`.

- If you are running IDMS on anything else, IDMS will search
...
: Prefix to be used in Redis for keys and channels. Useful for sharing one Redis server among multiple IDMS instances.

    Defaults to no prefix.

### Database

By default, IDMS uses **SQLite** with a database stored at `data/db.sqlite3`.
...
IDMS can make use of [Tika](https://tika.apache.org/) and
...
: Set the endpoint URL where IDMS can reach your Tika server.

    Defaults to "<http://localhost:9998>".

#### [`PAPERLESS_TIKA_GOTENBERG_ENDPOINT=<url>`](#PAPERLESS_TIKA_GOTENBERG_ENDPOINT) {#PAPERLESS_TIKA_GOTENBERG_ENDPOINT}

: Set the endpoint URL where IDMS can reach your Gotenberg server.

    Defaults to "<http://localhost:3000>".

If you run IDMS on docker, you can add those services to the
...
it exists and that the user running the IDMS service can
read/write its contents before you start IDMS.
...
: This is where IDMS stores all its data (search index, SQLite
...
: This is where your documents and thumbnails are stored.
    ...
    IDMS store all its data within the same volume.
...
: Changes the filenames IDMS uses to store documents in the media
...
: Tells IDMS to replace placeholders in
...
: This is where IDMS will store log files.

    Defaults to `PAPERLESS_DATA_DIR/log/`.

#### [`PAPERLESS_NLTK_DIR=<path>`](#PAPERLESS_NLTK_DIR) {#PAPERLESS_NLTK_DIR}

: This is where IDMS will search for the data required for NLTK
...
: This is where IDMS will store the classification model.
...
: **Required.** IDMS uses this to make session tokens and sign
sensitive data. IDMS will refuse to start if this is not set.
    ...
    !!! warning
        ...
        starting IDMS. Existing installations that relied on the
...
: If you're planning on putting IDMS on the open internet, then
...
: To host IDMS under a subpath url like example.com/paperless you
set this value to /paperless. No trailing slash!

    Defaults to none, which hosts IDMS at "/".

#### [`PAPERLESS_STATIC_URL=<path>`](#PAPERLESS_STATIC_URL) {#PAPERLESS_STATIC_URL}

: Override the STATIC_URL here. Unless you're hosting IDMS off a
...
If you do change it, be sure to include the trailing slash.
    ...
    !!! note
        ...
        When hosting IDMS behind a reverse proxy like Traefik or Nginx
...
: Specify a username here so that IDMS will automatically perform
login with the selected user.

    !!! danger

        Do not use this when exposing IDMS on the internet. There are
...
: If this environment variable is specified, IDMS automatically
...
: Specify a prefix that is added to the cookies used by IDMS to
identify the currently logged in user. This is useful for when
you're running two instances of IDMS on the same host.
...
applications.
    ...
    !!! warning
        ...
        external requests to your reverse-proxy to IDMS (that would
        effectively bypass all authentication).

        If you're exposing IDMS to the internet directly (i.e.
...
: Allow users to signup for a new IDMS account.
...
[IDMS settings](#PAPERLESS_EMAIL_HOST) and [the allauth docs](https://docs.allauth.org/en/latest/account/configuration.html)
...
IDMS uses [OCRmyPDF](https://ocrmypdf.readthedocs.io/en/latest/)
for performing OCR on documents and images. IDMS uses sensible
...
: Customize the language that IDMS will attempt to use when
parsing documents.
...
: Tell IDMS when and how to perform ocr on your documents. Four
modes are available:

    -   `auto` (default): IDMS detects whether a document already
    ...
    -   `redo`: IDMS will OCR all pages of your documents and
    ...
    -   `force`: IDMS rasterizes your documents, converting any
    ...
    -   `off`: IDMS never invokes the OCR engine. For PDFs, text
...
: Controls when IDMS creates a PDF/A archive version of your
...
: Tells IDMS to use `unpaper` to clean any input document before
...
: Tells IDMS to correct skewing (slight rotation of input images
...
: Tells IDMS to correct page rotation (90°, 180° and 270°
rotation).

    If you notice that IDMS is not rotating incorrectly rotated
...
: Specify the type of PDF documents that IDMS should produce.
    ...
    If not specified, `pdfa` is used. Remember that IDMS also keeps
    the original input file as well as the archived version.

#### [`PAPERLESS_OCR_PAGES=<num>`](#PAPERLESS_OCR_PAGES) {#PAPERLESS_OCR_PAGES}

: Tells IDMS to use only the specified amount of pages for OCR.
...
completely.
    ...
    `PAPERLESS_OCR_MODE=force`, IDMS will not modify any text it
...
: IDMS will OCR any images you put into the system and convert
them into PDF documents. This is useful if your scanner produces
images. In order to do so, IDMS needs to know the DPI of the
image. Most images from scanners will have this information embedded
and IDMS will detect and use that information. In case this
...
: IDMS will raise a warning when OCRing images which are over
...
but could result in missing text content.
    ...
    !!! note
        ...
        Increasing this limit could cause IDMS to consume additional
...
additional arguments you wish to pass to OCRmyPDF. Since IDMS
...
they use underscores instead of dashes.
    ...
    !!! warning
        ...
        IDMS has been tested to work with the OCR options provided
        above. There are many options that are incompatible with each other,
        so specifying invalid options may prevent IDMS from consuming
...
: IDMS does multiple things in the background: Maintain the
...
: Furthermore, IDMS uses multiple threads when consuming
documents to speed up OCR. This variable specifies how many pages
IDMS will process in parallel on a single document.
    ...
    !!! warning
        ...
        does not exceed your CPU core count or else IDMS will be
        extremely slow. If you want IDMS to process many documents in
        parallel, choose a high worker count. If you want IDMS to
        process very large documents faster, use a higher thread per worker
        count.

    If unset, IDMS uses `max(floor(cpu_count / PAPERLESS_TASK_WORKERS), 1)`
...
used during automatic classification. If disabled, IDMS will
...
Specifies which language IDMS should use when parsing dates from documents.
    ...
    If not set, IDMS will attempt to infer the language(s) from the OCR configuration (`PAPERLESS_OCR_LANGUAGE`).
...
: If not set, IDMS infers the language from
...
: As of version 3.0 IDMS allows duplicate documents to be consumed by default, _except_ when
this setting is enabled. When enabled, IDMS will check if a document with the same hash already
...
: Enable recursive watching of the consumption directory. IDMS
...
"bar" to the consumed file. IDMS will create any tags that
don't exist yet.
...
: After some initial validation, IDMS can trigger an arbitrary
...
: After a document is consumed, IDMS can trigger an arbitrary
...
: IDMS will check the document text for document date
...
: IDMS searches an entire document for dates. The first date
...
IDMS will also suggest other dates found in the document, up to
...
: IDMS creates thumbnails for plain text files by rendering the
...
: IDMS parses a document's creation date from filename and file
...
: IDMS will try to determine the document creation date from its
contents. Specify the date format IDMS should expect to see
...
: Configures how the consumer detects new files in the consumption directory.
    ...
    When set to `0` (default), IDMS uses native filesystem notifications for efficient, immediate detection of new files.

    When set to a positive number, IDMS polls the consumption directory at that interval in seconds. Use polling for network filesystems (NFS, SMB/CIFS) where native notifications may not work reliably.

    Defaults to 0.

#### [`PAPERLESS_CONSUMER_STABILITY_DELAY=<num>`](#PAPERLESS_CONSUMER_STABILITY_DELAY) {#PAPERLESS_CONSUMER_STABILITY_DELAY}

: Sets the time in seconds that a file must remain unchanged (same size and modification time) before IDMS will begin consuming it.
...
: The base URL for the OAuth callback. This is used to construct the full URL for the OAuth callback. This should be the URL that the IDMS instance is accessible at. If not set, defaults to the `PAPERLESS_URL` setting. At least one of these settings must be set to enable OAuth Email setup.
...
IDMS is used with the PATCH-T separator pages, users shouldn't
change this.
...
arrive.
    ...
    has been enabled. Note that IDMS will not automatically create the
    directory.
...
: Controls how often IDMS removes expired share link bundles (and their generated ZIP archives).
...
There are a few external software packages that IDMS expects to
...
actual user ID on the host system, which you can get by executing
    ...
    IDMS will change ownership on its folders to this user, so you
...
actual group ID on the host system, which you can get by executing
    ...
    IDMS will change ownership on its folders to this group, so you
    need to get this right in order to be able to write to the
    consumption directory.
...
: Additional OCR languages to install. By default, IDMS comes
...
: If set, overrides the default name "IDMS"
...
!!! note
    ...
    The logo file will be viewable by anyone with access to the IDMS instance login page,
...
: Enables the AI features in IDMS. This includes the AI-based
...
setting is required to be set to use the AI features.
    ...
    !!! note
        ...
        are responsible for any usage charges incurred by IDMS features, and your
        document data will be sent to the provider you configure.

        IDMS does not endorse any specific provider. Refer to your provider's terms of
...
: If set to false, IDMS blocks AI endpoint URLs that resolve to non-public addresses (e.g., localhost, etc).
