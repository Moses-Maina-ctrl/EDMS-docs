...
IDMS now ships with a fully-documented REST API and a browsable
...
2.  Session authentication
    ...
    When you're logged into IDMS in your browser, you're
...
3.  Token authentication
    ...
    IDMS also offers an endpoint to acquire authentication tokens.

    POST a username and password as a form or json string to
    `/api/token/` and IDMS will respond with a token, if the login
...
- Clients specify the specific version of the API they wish to use
  with every request and IDMS will handle the request using the
  specified API version.
- Even if the underlying data model changes, supported older API
  versions continue to serve compatible data.
- If no version is specified, IDMS serves the configured default
...
If an invalid version is specified, IDMS responds with
