...
- Ensure that the directory you're putting your documents in is the
  folder IDMS is watching. With docker, this setting is performed
  in the `docker-compose.yml` file. Without Docker, look at the
  `CONSUMPTION_DIR` setting. Don't adjust this setting if you're
  using docker.

- Ensure that redis is up and running. IDMS does its task
...
- Look at the output of IDMS and inspect it for any errors.
...
As an example, if you are running IDMS from any Ubuntu or
...
IDMS will manually check the consumption directory for changes
instead.

## IDMS always redirects to /admin

You probably had the old IDMS installed at some point. IDMS
...
This is required so that the user running IDMS inside docker has
...
This happens when certain dependencies of IDMS that are responsible
...
IDMS updates the training data.
...
data directory and let IDMS recreate it.
...
This happens when IDMS does not have permission to delete files
...
IDMS uses a search index to provide better and faster full text
...
that IDMS was unable to create and open these files.
...
This indicates that IDMS failed to read PDF metadata from one of
your documents. This happens when you open the affected documents in
IDMS for editing. IDMS will continue to work, and will simply
...
This probably indicates IDMS tried to consume the same file twice.
...
the same environment variable which is used by IDMS to optionally
...
This can happen during heavy consumption when using polling. IDMS will handle it correctly and the file
...
A user-maintained wiki page is available to help troubleshoot issues that may arise when trying to deploy IDMS on specific platforms, for example SELinux. Please see [the wiki](https://github.com/paperless-ngx/paperless-ngx/wiki/Platform%E2%80%90Specific-Troubleshooting).
