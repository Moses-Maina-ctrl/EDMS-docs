...
- Use the [document exporter](#exporter). The document exporter exports all your documents,
  ...
  documents and settings into a fresh instance of IDMS again or store your
...
!!! caution
    ...
    You cannot import the export generated with one version of IDMS in
...
- Backup the entire IDMS folder. This ensures that if your
  IDMS instance crashes at some point or your disk fails, you can
...
If you've backed-up IDMS using the [document exporter](#exporter),
...
## Updating IDMS {#updating}

!!! warning

    Please review the [migration instructions](migration-v3.md) before upgrading IDMS to v3.0, it includes some breaking changes that require manual intervention before upgrading.
...
After that, ensure that IDMS is stopped:
...
1.  Update dependencies. New IDMS version may require additional
...
3.  Migrate the database.
    ...
    This might not actually do anything. Not every new IDMS version
...
IDMS is compatible with Django-supported versions of PostgreSQL and MariaDB and it is generally
...
!!! note
    ...
    As of IDMS v2.18, the minimum supported version of PostgreSQL is 14.
...
IDMS comes with some management commands that perform various
maintenance tasks on your IDMS instance. You can invoke these
commands in the following way:

With Docker Compose, while IDMS is running:
...
With docker, while IDMS is running:
...
and database contents) from IDMS into a folder for backup or
...
If the target directory already exists and contains files, IDMS
will assume that the contents of the export directory are a previous
export and will attempt to update the previous export. IDMS will
only export changed and added files. IDMS determines whether a file
has changed by inspecting the file attributes "date/time modified" and
"size". If that does not work out for you, specify `-c` or
`--compare-checksums` and IDMS will attempt to compare file
...
IDMS will not remove any existing files in the export directory. If
you want IDMS to also remove files that do not belong to the
current export such as files from deleted documents, specify `-d` or `--delete`.
Be careful when pointing IDMS to a directory that already contains
other files.

The filenames generated by this command follow the format
`[date created] [correspondent] [title].[extension]`. If you want
IDMS to use [`PAPERLESS_FILENAME_FORMAT`](configuration.md#PAPERLESS_FILENAME_FORMAT) for exported filenames
...
the `export` folder in your IDMS source directory. Specify
`../export` as the `source`.

!!! note

    Importing from a previous version of IDMS may work, but for best
    results it is suggested to match the versions.

!!! warning

    The importer should be run against a completely empty installation (database and directories) of IDMS.
...
IDMS has a built-in sanity checker that inspects your document
...
IDMS automatically fetches your e-mail every 10 minutes by default.
...
IDMS stores archived PDF/A documents alongside your original
...
version of IDMS, your documents won't have archived versions.
...
IDMS already catches and warns of exactly matching documents,
...
If the audit log is enabled IDMS keeps an audit log of all changes made to documents. Functionality to automatically remove entries for deleted documents was added but
