...
!!! tip "Quick Start"
    ...
    If you just want IDMS running quickly, use our installation script:
    ```shell-session
...
IDMS provides an interactive script for Docker Compose setups.
...
IDMS should be available at `http://127.0.0.1:8000` (or similar,
...
Your IDMS instance should now be accessible at
...
If you want to run IDMS as a rootless container, set `user:` in `docker-compose.yml` to the UID and GID of your host user (use `id -u` and `id -g` to find these values). The container process starts directly as that user with no internal privilege remapping:
...
file system, IDMS will not pick up new files with the default
...
- IDMS runs on Linux only, Windows is not supported.
- Python 3.11, 3.12, 3.13, or 3.14 is required. As a policy, IDMS aims to support at least the three most recent Python versions and drops support for versions as they reach end-of-life. Newer versions may work, but some dependencies may not be fully compatible.

#### Installation

1.  Install dependencies. IDMS requires the following packages:
    - `python3`
    - `python3-pip`
    - `python3-dev`
...
3.  Optional: Install `postgresql` and configure a database, user, and
    password for IDMS. If you do not wish to use PostgreSQL,
...
6.  Configure IDMS. See [configuration](configuration.md) for details.
    Edit the included `paperless.conf` and adjust the settings to your
    needs. Required settings for getting IDMS running are:
    ...
    - [`PAPERLESS_CONSUMPTION_DIR`](configuration.md#PAPERLESS_CONSUMPTION_DIR) should point to the folder
      that IDMS should watch for incoming documents.
      Likewise, [`PAPERLESS_DATA_DIR`](configuration.md#PAPERLESS_DATA_DIR) and
      [`PAPERLESS_MEDIA_ROOT`](configuration.md#PAPERLESS_MEDIA_ROOT) define where IDMS stores its data.
...
10. Optional: Test that IDMS is working by running
    ...
    accessing from the same device on which IDMS is installed.
    ...
    !!! tip
        ...
        This will not start the consumer. IDMS does this in a separate
        process.

11. Set up systemd services to run IDMS automatically. You may use
...
12. Configure ImageMagick to allow processing of PDF documents. Most
    ...
    contain malware. If you don't do this, IDMS will fall back to
...
Your IDMS instance should now be accessible at `http://localhost:8000` (or similar, depending on your configuration).
...
You can migrate to IDMS from Paperless-ng or from the original
IDMS project.

<h3 id="migration_ng">Migrating from Paperless-ng</h3>

IDMS is meant to be a drop-in replacement for Paperless-ng, and
...
<h3 id="migration_paperless">Migrating from IDMS</h3>

At its core, IDMS is still IDMS and fully compatible.
However, some things have changed under the hood, so you need to adapt
your setup depending on how you installed IDMS.

This section describes how to update an existing IDMS Docker
installation. Keep these points in mind:

- Read the [changelog](changelog.md) and
  take note of breaking changes.
- Decide whether to stay on SQLite or migrate to PostgreSQL.
  Both work fine with IDMS.
  However, if you already have a database server running
  for other services, you might as well use it for IDMS as well.
- The task scheduler of IDMS, which is used to execute periodic
...
Migration to IDMS is then performed in a few simple steps:
...
3.  Download the latest release of IDMS. You can either use
    the Docker Compose files from
    [here](https://github.com/paperless-ngx/paperless-ngx/tree/main/docker/compose)
    or clone the repository to build the image yourself (see
    [development docs](development.md#docker_build)). You can either replace your current IDMS
    folder or put IDMS in
    a different location.

    !!! warning

        IDMS includes a `.env` file. This will set the project name
        for Docker Compose to `paperless`, which will also define the
        volume names created by IDMS. However, if you notice that
        paperless-ngx is not using your old IDMS volumes, verify the
...
6.  Follow the update procedure in [Update IDMS](administration.md#updating).
...
7.  In order to find your existing documents with the new search
    ...
    that, IDMS will maintain the index automatically.
...
8.  Start IDMS.
    ...
    This will run IDMS in the background and automatically start it
    on system boot.

9.  IDMS may have installed a permanent redirect to `admin/` in your
...
1.  Stop and remove the IDMS container.
...
## Running IDMS on less powerful devices {#less-powerful-devices data-toc-label="Less Powerful Devices"}

IDMS runs on Raspberry Pi. Some tasks can be slow on lower-powered
...
- Consider setting [`PAPERLESS_OCR_PAGES`](configuration.md#PAPERLESS_OCR_PAGES) to 1, so that IDMS
...
- [`PAPERLESS_TASK_WORKERS`](configuration.md#PAPERLESS_TASK_WORKERS) and [`PAPERLESS_THREADS_PER_WORKER`](configuration.md#PAPERLESS_THREADS_PER_WORKER) are
  ...
  cores, meaning that IDMS will use 2 workers and 2 threads per
...
- Keep [`PAPERLESS_OCR_MODE`](configuration.md#PAPERLESS_OCR_MODE) at its default value `auto` and consider
  OCRing your documents before feeding them into IDMS. Some
...
**Using a reverse proxy with IDMS**

Please see [the wiki](https://github.com/paperless-ngx/paperless-ngx/wiki/Using-a-Reverse-Proxy-with-Paperless-ngx#nginx) for user-maintained documentation on using nginx with IDMS.

**Enhancing security**

Please see [the wiki](https://github.com/paperless-ngx/paperless-ngx/wiki/Using-Security-Tools-with-Paperless-ngx) for user-maintained documentation on configuring security tools like Fail2ban with IDMS.
