...
## _What's the general plan for IDMS?_

**A:** While IDMS is already considered largely
...
- IDMS never modifies your original documents. It keeps
  checksums of all documents and uses a scheduled sanity checker to
  check that they remain the same.
- By default, IDMS uses the internal ID of each document as its
  filename. This might not be very convenient for export. However, you
  can adjust the way files are stored in IDMS by
  [configuring the filename format](advanced_usage.md#file-name-handling).
- [The exporter](administration.md#exporter) is
  another easy way to get your files out of IDMS with reasonable
  file names.
...
- With the optional Tika integration enabled (see [Tika configuration](https://docs.paperless-ngx.com/configuration#tika)),
  IDMS also supports various Office documents (.docx, .doc, odt,
  .ppt, .pptx, .odp, .xls, .xlsx, .ods).

IDMS determines the type of a file by inspecting its content
...
The long answer is that certain parts of IDMS will run very slow,
such as the OCR. On Raspberry Pi, try to OCR documents before feeding
them into IDMS so that IDMS can reuse the text. The web
interface is a lot snappier, since it runs in your browser and IDMS
has to do much less work to serve the data.

!!! note

    You can adjust some of the settings so that IDMS uses less
...
!!! note
    ...
    For ARMv7 (32-bit) systems, IDMS may still function, but it could require
...
**A:** IDMS is available as [community
...
able to run IDMS, you're a bit on your own. If you can't run the
